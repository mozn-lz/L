...
router.get('/login', function(req, res, next) {
    res.render('login', { user: req.session.user, title: 'New product', page: 'Home', role: '' });
});
router.get('/register', function(req, res, next) {
    res.render('register', { user: req.session.user, title: 'New product', page: 'Home', role: '' });
});
...
let fn_findUser = (user) => {
    return new Promise((resolve, reject) => {
        db_read(table, { cell: user.cell }, cellUser => {
...
router.post('/users', (req, res, next) => {
    ...
    if (req.body.register) {
        ...
        let new_user = {
            ...
            cell1: req.body.cell1.trim(),
            cell2: req.body.cell2.trim(),
            role: false,
            password: req.body.password
        };

        if (!check_email(new_user.email) || !check_tel(new_user.cell2) || 
            !check_tel(new_user.cell1) || !check_name(new_user.name) || 
            !check_name(new_user.surname) || !check_psswd(new_user.password) ||
